...
- **E2E Stability:** Playwright tests run in Chromium and WebKit. For auth setup, prefer `page.addInitScript()` before navigation instead of visiting a page and then mutating `localStorage`; WebKit is sensitive to navigation races. Avoid `networkidle` for Next.js dev-server flows; prefer page assertions after `domcontentloaded`. Hidden submenu items must use real non-visibility (`hidden`/unmounted), not only `max-height` or `opacity`, because Playwright can still treat them as visible.

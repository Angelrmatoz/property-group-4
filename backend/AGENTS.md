...
- **Docker & Oracle Cloud:** The backend is containerized holding to the standard `Dockerfile` provided. Automatic CI/CD builds are handled by GitHub Actions (`.github/workflows/deploy-backend.yml`), which pushes the image to GHCR. Manual pull and restart on the Oracle Cloud VM is required after each build.
  - Do not alter the `.yml` workflow without ensuring compatibility with the GHCR (GitHub Container Registry) tags.
  - The production server runs on an Oracle Cloud free-tier VM, Node.js 20 minimum.
  - Keep `console.log` statements minimal to avoid swelling logs.
